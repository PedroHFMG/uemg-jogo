...
func _ready():
	scale_self = scale
	set_meta("paralelogramo-3-" + str(stepify(rotation_degrees, 0.01)), true)
...
func _on_Area2D_area_entered(area):
	if area.has_meta("paralelogramo-3-vazio-" + str(stepify(rotation_degrees, 0.01))) and (area.scale + scale_math) == scale:
		encaixe = true
		obj.append(area)
