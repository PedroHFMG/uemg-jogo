...
func _ready():
	scale_self = scale
	set_meta("losango-" + str(stepify(rotation_degrees, 0.01)), true)
...
func _on_Area2D_area_entered(area):
	if area.has_meta("losango-vazio-" + str(stepify(rotation_degrees, 0.01))) and (area.scale + scale_math) == scale:
		encaixe = true
		obj.append(area)
